...
export const fetchWorkspacesByAccountId = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const response = apiResponse.success(
      HttpStatusCode.OK,
      result,
      `Fetched workspaces: ${result.length}`
    )
...
export const updateWorkspaceProfile = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!bodyValidation.success) {
      const response = apiResponse.error(
        HttpErrors.ValidationFailed(
          `Profile name validation failed: ${bodyValidation.error.message}`
        )
      )
      res.status(response.code).send(response)
      return
...
export const deleteWorkspace = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const [workspace] = await db
    .select()
    .from(workspaces)
    .where(eq(workspaces.uuid, workspaceId))
    .limit(1)
